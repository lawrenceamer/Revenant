...
// File generated on 18/12/2011 14:56:52 from Type Library described below.

// ************************************************************************  //
// Type Lib: C:\Documents and Settings\Ludo\Mes documents\Lazarus Projects\test\com\taskschd.dll (1)
...
//   Hint: Member 'Type' of 'ITrigger' changed to 'Type_'
//   Hint: Parameter 'Type' of IActionCollection.Create changed to 'Type_'
//   Hint: Member 'Type' of 'IAction' changed to 'Type_'
//   Hint: Member 'To' of 'IEmailAction' changed to 'To_'
//   Error creating palette bitmap of (TTaskScheduler) : No Server registered for this CoClass
//   Error creating palette bitmap of (TTaskHandlerPS) : No Server registered for this CoClass
//   Error creating palette bitmap of (TTaskHandlerStatusPS) : No Server registered for this CoClass
// ************************************************************************ //
// *************************************************************************//
// NOTE:                                                                      
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  TTaskSchedulerProperties= class;
{$ENDIF}
  TTaskScheduler = class(TOleServer)
  private
    FIntf:        ITaskService;
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    FProps:       TTaskSchedulerProperties;
    function      GetServerProperties: TTaskSchedulerProperties;
{$ENDIF}
    function      GetDefaultInterface: ITaskService;
  protected
    procedure InitServerData; override;
    function Get_Connected: WordBool;
    function Get_TargetServer: WideString;
    function Get_ConnectedUser: WideString;
    function Get_ConnectedDomain: WideString;
    function Get_HighestVersion: LongWord;
  public
    constructor Create(AOwner: TComponent); override;
    destructor  Destroy; override;
    procedure Connect; override;
    procedure ConnectTo(svrIntf: ITaskService);
    ...
    procedure Connect1(serverName: OleVariant; user: OleVariant); overload;
    procedure Connect1(serverName: OleVariant; user: OleVariant; domain: OleVariant); overload;
    procedure Connect1(serverName: OleVariant; user: OleVariant; domain: OleVariant; 
                       password: OleVariant); overload;
    property DefaultInterface: ITaskService read GetDefaultInterface;
    property Connected: WordBool read Get_Connected;
    property TargetServer: WideString read Get_TargetServer;
    property ConnectedUser: WideString read Get_ConnectedUser;
    property ConnectedDomain: WideString read Get_ConnectedDomain;
  ...
  published
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    property Server: TTaskSchedulerProperties read GetServerProperties;
{$ENDIF}
  end;
...
// *********************************************************************//
 TTaskSchedulerProperties = class(TPersistent)
  private
    FServer:    TTaskScheduler;
    function    GetDefaultInterface: ITaskService;
    constructor Create(AServer: TTaskScheduler);
  protected
    function Get_Connected: WordBool;
    function Get_TargetServer: WideString;
    function Get_ConnectedUser: WideString;
...
// *********************************************************************//
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  TTaskHandlerPSProperties= class;
{$ENDIF}
  TTaskHandlerPS = class(TOleServer)
  private
    FIntf:        ITaskHandler;
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    FProps:       TTaskHandlerPSProperties;
    function      GetServerProperties: TTaskHandlerPSProperties;
{$ENDIF}
    function      GetDefaultInterface: ITaskHandler;
  protected
    procedure InitServerData; override;
  public
    constructor Create(AOwner: TComponent); override;
    destructor  Destroy; override;
    procedure Connect; override;
    procedure ConnectTo(svrIntf: ITaskHandler);
    procedure Disconnect; override;
    function Start(const pHandlerServices: IUnknown; const Data: WideString): HResult;
    function Stop(out pRetCode: HResult): HResult;
    function Pause: HResult;
    function Resume: HResult;
    property DefaultInterface: ITaskHandler read GetDefaultInterface;
  published
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    property Server: TTaskHandlerPSProperties read GetServerProperties;
{$ENDIF}
  end;
...
//  of the properties of this server)
// *********************************************************************//
 TTaskHandlerPSProperties = class(TPersistent)
  private
    FServer:    TTaskHandlerPS;
    function    GetDefaultInterface: ITaskHandler;
    constructor Create(AServer: TTaskHandlerPS);
  protected
  public
    property DefaultInterface: ITaskHandler read GetDefaultInterface;
  published
...
// *********************************************************************//
// OLE Server Proxy class declaration
// Server Object    : TTaskHandlerStatusPS
// Help String      : 
...
// *********************************************************************//
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  TTaskHandlerStatusPSProperties= class;
{$ENDIF}
  TTaskHandlerStatusPS = class(TOleServer)
  private
    FIntf:        ITaskHandlerStatus;
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    FProps:       TTaskHandlerStatusPSProperties;
    function      GetServerProperties: TTaskHandlerStatusPSProperties;
{$ENDIF}
    function      GetDefaultInterface: ITaskHandlerStatus;
  protected
    procedure InitServerData; override;
  public
    constructor Create(AOwner: TComponent); override;
    destructor  Destroy; override;
    procedure Connect; override;
    procedure ConnectTo(svrIntf: ITaskHandlerStatus);
    procedure Disconnect; override;
    function UpdateStatus(percentComplete: Smallint; const statusMessage: WideString): HResult;
    function TaskCompleted(taskErrCode: HResult): HResult;
    property DefaultInterface: ITaskHandlerStatus read GetDefaultInterface;
  published
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    property Server: TTaskHandlerStatusPSProperties read GetServerProperties;
{$ENDIF}
  end;
...
//  of the properties of this server)
// *********************************************************************//
 TTaskHandlerStatusPSProperties = class(TPersistent)
  private
    FServer:    TTaskHandlerStatusPS;
    function    GetDefaultInterface: ITaskHandlerStatus;
    constructor Create(AServer: TTaskHandlerStatusPS);
  protected
  public
    property DefaultInterface: ITaskHandlerStatus read GetDefaultInterface;
  published
...
procedure TTaskScheduler.InitServerData;
const
  CServerData: TServerData = (
    ClassID:   '{0F87369F-A4E5-4CFC-BD3E-73E6154572DD}';
    IntfIID:   '{2FABA4C7-4DA9-4013-9697-20CC3FD40F85}';
    EventIID:  '';
    LicenseKey: nil;
    Version: 500);
begin
...
procedure TTaskScheduler.Connect;
var
  punk: IUnknown;
begin
  if FIntf = nil then
  begin
    punk := GetServer;
    Fintf:= punk as ITaskService;
  end;
end;

procedure TTaskScheduler.ConnectTo(svrIntf: ITaskService);
begin
  Disconnect;
  FIntf := svrIntf;
end;

procedure TTaskScheduler.DisConnect;
begin
  if Fintf <> nil then
  begin
    FIntf := nil;
...
function TTaskScheduler.GetDefaultInterface: ITaskService;
begin
  if FIntf = nil then
    Connect;
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps := TTaskSchedulerProperties.Create(Self);
{$ENDIF}
end;
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
function TTaskScheduler.GetServerProperties: TTaskSchedulerProperties;
begin
  Result := FProps;
end;
{$ENDIF}

function TTaskScheduler.Get_Connected: WordBool;
begin
    Result := DefaultInterface.Connected;
end;

function TTaskScheduler.Get_TargetServer: WideString;
begin
    Result := DefaultInterface.TargetServer;
end;

function TTaskScheduler.Get_ConnectedUser: WideString;
begin
    Result := DefaultInterface.ConnectedUser;
end;

function TTaskScheduler.Get_ConnectedDomain: WideString;
begin
    Result := DefaultInterface.ConnectedDomain;
end;

function TTaskScheduler.Get_HighestVersion: LongWord;
begin
    Result := DefaultInterface.HighestVersion;
end;

function TTaskScheduler.GetFolder(const Path: WideString): ITaskFolder;
begin
  Result := DefaultInterface.GetFolder(Path);
end;

function TTaskScheduler.GetRunningTasks(flags: Integer): IRunningTaskCollection;
begin
  Result := DefaultInterface.GetRunningTasks(flags);
end;

function TTaskScheduler.NewTask(flags: LongWord): ITaskDefinition;
begin
  Result := DefaultInterface.NewTask(flags);
end;

procedure TTaskScheduler.Connect1;
begin
  DefaultInterface.Connect(EmptyParam, EmptyParam, EmptyParam, EmptyParam);
end;

procedure TTaskScheduler.Connect1(serverName: OleVariant);
begin
  DefaultInterface.Connect(serverName, EmptyParam, EmptyParam, EmptyParam);
end;

procedure TTaskScheduler.Connect1(serverName: OleVariant; user: OleVariant);
begin
  DefaultInterface.Connect(serverName, user, EmptyParam, EmptyParam);
end;

procedure TTaskScheduler.Connect1(serverName: OleVariant; user: OleVariant; domain: OleVariant);
begin
  DefaultInterface.Connect(serverName, user, domain, EmptyParam);
end;

procedure TTaskScheduler.Connect1(serverName: OleVariant; user: OleVariant; domain: OleVariant; 
                                  password: OleVariant);
begin
  DefaultInterface.Connect(serverName, user, domain, password);
end;

{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
constructor TTaskSchedulerProperties.Create(AServer: TTaskScheduler);
begin
  inherited Create;
  FServer := AServer;
end;

function TTaskSchedulerProperties.GetDefaultInterface: ITaskService;
begin
  Result := FServer.DefaultInterface;
end;

function TTaskSchedulerProperties.Get_Connected: WordBool;
begin
    Result := DefaultInterface.Connected;
end;

function TTaskSchedulerProperties.Get_TargetServer: WideString;
begin
    Result := DefaultInterface.TargetServer;
end;

function TTaskSchedulerProperties.Get_ConnectedUser: WideString;
begin
    Result := DefaultInterface.ConnectedUser;
end;

function TTaskSchedulerProperties.Get_ConnectedDomain: WideString;
begin
    Result := DefaultInterface.ConnectedDomain;
end;

function TTaskSchedulerProperties.Get_HighestVersion: LongWord;
begin
    Result := DefaultInterface.HighestVersion;
end;
...
procedure TTaskHandlerPS.InitServerData;
const
  CServerData: TServerData = (
    ClassID:   '{F2A69DB7-DA2C-4352-9066-86FEE6DACAC9}';
    IntfIID:   '{839D7762-5121-4009-9234-4F0D19394F04}';
    EventIID:  '';
    LicenseKey: nil;
    Version: 500);
begin
...
procedure TTaskHandlerPS.Connect;
var
  punk: IUnknown;
begin
  if FIntf = nil then
  begin
    punk := GetServer;
    Fintf:= punk as ITaskHandler;
  end;
end;

procedure TTaskHandlerPS.ConnectTo(svrIntf: ITaskHandler);
begin
  Disconnect;
  FIntf := svrIntf;
end;

procedure TTaskHandlerPS.DisConnect;
begin
  if Fintf <> nil then
  begin
    FIntf := nil;
...
function TTaskHandlerPS.GetDefaultInterface: ITaskHandler;
begin
  if FIntf = nil then
    Connect;
  Assert(FIntf <> nil, 'DefaultInterface is NULL. Component is not connected to Server. You must call ''Connect'' or ''ConnectTo'' before this operation');
...
constructor TTaskHandlerPS.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps := TTaskHandlerPSProperties.Create(Self);
{$ENDIF}
end;

destructor TTaskHandlerPS.Destroy;
begin
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps.Free;
{$ENDIF}
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
function TTaskHandlerPS.GetServerProperties: TTaskHandlerPSProperties;
begin
  Result := FProps;
end;
{$ENDIF}

function TTaskHandlerPS.Start(const pHandlerServices: IUnknown; const Data: WideString): HResult;
begin
  Result := DefaultInterface.Start(pHandlerServices, Data);
end;

function TTaskHandlerPS.Stop(out pRetCode: HResult): HResult;
begin
  Result := DefaultInterface.Stop(pRetCode);
end;

function TTaskHandlerPS.Pause: HResult;
begin
  Result := DefaultInterface.Pause;
end;

function TTaskHandlerPS.Resume: HResult;
begin
  Result := DefaultInterface.Resume;
end;

{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
constructor TTaskHandlerPSProperties.Create(AServer: TTaskHandlerPS);
begin
  inherited Create;
  FServer := AServer;
end;

function TTaskHandlerPSProperties.GetDefaultInterface: ITaskHandler;
begin
  Result := FServer.DefaultInterface;
end;
...
procedure TTaskHandlerStatusPS.InitServerData;
const
  CServerData: TServerData = (
    ClassID:   '{9F15266D-D7BA-48F0-93C1-E6895F6FE5AC}';
    IntfIID:   '{EAEC7A8F-27A0-4DDC-8675-14726A01A38A}';
    EventIID:  '';
    LicenseKey: nil;
    Version: 500);
begin
...
procedure TTaskHandlerStatusPS.Connect;
var
  punk: IUnknown;
begin
  if FIntf = nil then
  begin
    punk := GetServer;
    Fintf:= punk as ITaskHandlerStatus;
  end;
end;

procedure TTaskHandlerStatusPS.ConnectTo(svrIntf: ITaskHandlerStatus);
begin
  Disconnect;
  FIntf := svrIntf;
end;

procedure TTaskHandlerStatusPS.DisConnect;
begin
  if Fintf <> nil then
  begin
    FIntf := nil;
...
function TTaskHandlerStatusPS.GetDefaultInterface: ITaskHandlerStatus;
begin
  if FIntf = nil then
    Connect;
  Assert(FIntf <> nil, 'DefaultInterface is NULL. Component is not connected to Server. You must call ''Connect'' or ''ConnectTo'' before this operation');
...
constructor TTaskHandlerStatusPS.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps := TTaskHandlerStatusPSProperties.Create(Self);
{$ENDIF}
end;

destructor TTaskHandlerStatusPS.Destroy;
begin
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps.Free;
{$ENDIF}
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
function TTaskHandlerStatusPS.GetServerProperties: TTaskHandlerStatusPSProperties;
begin
  Result := FProps;
end;
{$ENDIF}

function TTaskHandlerStatusPS.UpdateStatus(percentComplete: Smallint; 
                                           const statusMessage: WideString): HResult;
begin
  Result := DefaultInterface.UpdateStatus(percentComplete, statusMessage);
end;

function TTaskHandlerStatusPS.TaskCompleted(taskErrCode: HResult): HResult;
begin
  Result := DefaultInterface.TaskCompleted(taskErrCode);
end;

{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
constructor TTaskHandlerStatusPSProperties.Create(AServer: TTaskHandlerStatusPS);
begin
  inherited Create;
  FServer := AServer;
end;

function TTaskHandlerStatusPSProperties.GetDefaultInterface: ITaskHandlerStatus;
begin
  Result := FServer.DefaultInterface;
end;
...
procedure Register;
begin
  RegisterComponents(dtlServerPage, [TTaskScheduler, TTaskHandlerPS, TTaskHandlerStatusPS]);
end;
